...
\blue{
    ...
    \quad In the above Fig.~\ref{fig:discrete_receiver}, the mutual information of traditional MIMO discrete receivers and EIT continuous-space receivers are compared. On one hand, the continuous-space mutual information is calculated by applying appropriate numerical schemes to the definition of Fredholm determinant $I=\log\det({\bf 1}+T_{\bf E}T_{\bf N}^{-1})$, where the self-adjoint autocorrelation operator $T_{\bf E}$ describes the statistical knowledge about the received electric field, and similarly, $T_{\bf N}$ describes that about the noise field. The received field autocorrelation $T_{\bf E}$ is calculated from the transmitted current ${\bf J}$ and the deterministic Green's function ${\bf G}({\bf r}, {\bf s})$ as the EM channel. On the other hand, the discrete MIMO mutual information is evaluated directly by treating the Green's function ${\bf G}({\bf r}, {\bf s})$ as the MIMO channel coefficient. With equal received SNR, in this figure, the continuous EIT mutual information is plotted as dashed lines, and the discrete MIMO mutual information is plotted as bold lines with triangles of the same color. 
