...
\textbf{Reviewer 1:}
In the open problem section VI.D, it says that the achievability of capacity is not proved. Then typically in information theory, this means that the capacity is not found at all. Capacity has an operational meaning, which is the highest rate. It may or may not be the maximum mutual information, depending on the channel assumption (which appears core for this new area of electromagnetic IT). So it appears to me that [9] [11] at most provide an upper bound (converse) for the capacity and the capacity is an open problem. Additionally, the claims on the hardness of achievability or converse proofs are purely subjective. When a problem is open, everything could be hard. When the answer is known, it is not easy to reach a consensus on which part is harder (in IT, it could be more widely believed that converse is harder).

\blue{\textbf{Authors: } 
Many thanks for pointing out these limitations in {\bf Section VI-D} of this paper. As mentioned by the reviewer, the ``capacity'' has an operational meaning, and the maximum mutual information does not necessarily equal the ``capacity''. Thus, the EM capacity has not been found in the strict sense. We agree with the reviewer that the existing works about the EM capacity is still in its infancy. However, substantial progresses have been made in the literature about the EM capacity, which will be summarized into two parts as follows. 

1.1) {\bf Achievable Rates.} By assuming a linear time-invariant propagation environment, the EM channel is modeled by a spatio-temporal (4-dimensional) linear Green's function. This Green's function is viewed as a linear operator that maps the current distribution on the transmitter (source) to the receiver (destination). Since any (bounded) linear operator admits to the singular value decomposition, the Green's function is then decomposed into a series of eigenmodes [R1-2] before performing water-filling on the decomposed parallel sub-channels. 
...
1.2) {\bf Converse Bounds.} As is mentioned by the reviewer, converse bounds can be obtained by evaluating the maximum ``continuous'' mutual information between the source and destination regions. Since the source current distributions and destination electric field distributions are considered as random fields ${\bm J}({\bm s})$ and ${\bm E}({\bm r})$, a new mutual information quantity between two random fields are defined [R3]. 

We have carefully checked the existing works in the literature, and have found that there 

In the revised paper, we have rewritten this subsection as 

\quad We believe that the existing works such as According to Fano's inequality, if there is an $(n,\epsilon)$ code of rate $R$, then $R\leq I(X;Y)$. 
In the revised paper, we have rewritten this subsection as 

The main difficulty of the evaluation of the EM capacity is the infinite 
...
[R1] F. K. Gruber, and E. A. Marengo, ``New aspects of electromagnetic information theory for wireless and antenna systems,'' {\it IEEE Trans. Antenna Propag.}, vol. 56, no. 11, pp. 3470-3484, Nov. 2008. 

[R2] M. A. Jensen, and J. W. Wallace, ``Capacity of the continuous-space electromagnetic channel,'' {\it IEEE Trans. Antenna Propag.}, vol. 56, no. 2, pp. 524-531, Feb. 2008. 
