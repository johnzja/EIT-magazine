...
\blue{This is because the entries of the channel matrix can be calculated as ${\bm H}_{qp}=\langle\psi_q|T|\phi_p \rangle$, where $\phi_p$ and $\psi_q$ are the $p$-th and $q$-th eigenmodes of the transmit and receive antennas, respectively. Thus, modeling the EM channel by a bounded linear operator $T$ is mathematically consistent with the existing matrix modeling. }
