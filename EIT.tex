...
By contrast, in the information theory community, the DoF has a mathematically rigorous definition that is directly originated from the spectral theory in functional analysis. 
An important example would be the evaluation of the DoF of a waveform channel bandlimited to $[-W, W]\, {\rm Hz}$. Equivalently, we want to know how many real numbers can be communicated per unit time through this $W$-bandlimited channel. This analysis can be done by collecting all the $W$-bandlimited signals together into a functional space $\mathcal{B}_W$, and asking at least how many coefficients $\{x_n\}_{n=1}^{N_\epsilon}$ are needed to describe an arbitrary $f(t)\in \mathcal{B}_W$ up to a precision of $\epsilon$, i.e., $\|f(t)-\sum_{n}x_n\phi_n(t)\|/\|f\|\leq \epsilon$, where $\phi_n$ are basis functions that are chosen from $\mathcal{B}_W$. This minimum number of coefficients $N_\epsilon$ is called the Kolmogorov N-width $d_N(\mathcal{B}_W)$ of the functional space $\mathcal{B}_W$ under given norm $\|\cdot\|$. If the norm is $L^2(\mathbb{R})$, then $N_0=+\infty$, since a bandlimited signal observed on the infinite time axis $t\in \mathbb{R}$ can carry an infinite number of real samples. However, if the signal $f(t)$ is only observed for a finite time interval $[-T/2, T/2]$, i.e., the norm is chosen to be $L^2(-T/2, T/2)$, then $N_\epsilon=2WT+\mathcal{O}(\log (WT))$ holds for any given $\epsilon>0$. This means that this $W$-bandlimited channel can essentially transmit $2WT$ real numbers within time $T$. In this case, the optimal basis waveforms $\phi_n(t)$ are prolate spheroidal wave functions (PSWFs), as is shown in Fig.~\ref{fig:PSWF}. 
