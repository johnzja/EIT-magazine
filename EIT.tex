...
\begin{abstract}
	Traditional single-input single-output (SISO) and multiple-input multiple-output (MIMO) information theory adopt spatially discrete modeling, which mismatches the continuous nature of the underlying electromagnetic (EM) fields. % Discrete analysis may fail to reveal the fundamental information transmission limit of a continuous wireless system.    
 	Therefore, it is essential to analyze the information-carrying capability of continuous EM fields, which motivates the research of EM information theory (EIT). In this article, we systematically investigate the basics and results of EIT. First, we review the fundamental analysis tools of classical information theory and EM theory. Then, we introduce the modeling and analysis methodologies of EIT, including continuous field modeling, degree of freedom (DoF), mutual information, and capacity analyses. After that, several EIT-inspired applications are discussed to illustrate how EIT guides the design of practical wireless systems.  Finally, we point out several open problems of EIT, where further research efforts are required for EIT to construct a unified interdisciplinary theory.
\end{abstract}

\begin{IEEEkeywords}
    Electromagnetic information theory (EIT), electromagnetic channel modeling, random field, degree of freedom (DoF), capacity.  
\end{IEEEkeywords}
...
Therefore, a theory that can model and analyze the real wireless information system with continuous EM fields is necessary, which motivates the research of EM information theory (EIT).

EIT is an interdisciplinary subject that integrates deterministic physical theory and statistical mathematical theory to study the information transmission mechanism in the spatially continuous EM field. 
Specifically, EIT unifies the basic laws and methodologies in both classical EM theory and information theory. 
As a result, EIT is capable of building the framework of system modeling and performance analysis that fits the EM propagation theory for wireless information systems. 
...
\item{Finally, several open problems of EIT are pointed out, such as the analog and digital challenges brought by EIT, the compatible EM noise modeling, the physically appropriate power constraint, and the achievability of the EM capacity. We explain the meanings of these open problems, and why they are important for the future research of EIT. }
\end{itemize}
...
Utilizing the Karhunen-Lo\`{e}ve (KL) expansion theorem, the continuous channel can be decomposed into parallel sub-channels. 
...
The theory of EIT possibly predicts a substantial capacity improvement as long as the transceivers are equipped with fully-continuous antenna surfaces. However, the implementation of such spatially continuous antenna surfaces will inevitably require the processing of an enormous volume of baseband data as well as a vast amount of electrically small antenna patches. Thus, the exploding number of antennas will bring fundamentally new challenges to both the digital and analog signal processing. 

In the digital domain, the optimal current distribution patterns should be frequently updated within a typical timescale of 1\,ms to accommodate the fast-varying EIT channel. In addition, the DFT bases have to be further improved since the EIT eigenmodes are generally not sinusoidal~\cite{marzetta2022fourier}. Thus, numerous nearly-continuous bases have to be digitally designed in a frequent manner, which is a great challenge even to the state-of-the-art baseband processors.  

In the analog domain, there exists a physical tradeoff between the performance and the size of the antenna~\cite{stratton2007electromagnetic}. Thus, it is practically difficult to design near-continuous antennas without sacrificing the radiation efficiency and working bandwidth. Apart from the physical tradeoff, the mutual coupling between adjacent antennas need to be carefully suppressed when the antenna array becomes denser, in order not to churn the designed radiation patterns and degrade the communication performance. 
...
In classical information theory, to establish a ``capacity'' value, one needs to prove an achievability theorem and a converse bound. Only when these two values coincide can one claim that such a value can be called as ``capacity''. Usually, the proof of the achievability theorem is more difficult than the coverse theorem, which requires ingenious mathematical constructions~\cite{shannon1948mathematical}. 
...
The recent progress on EIT has demonstrated its possibility to become a unified and widely applicable theory. However, there are still some unresolved open problems that require further study in the future. 
