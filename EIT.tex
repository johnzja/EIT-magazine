...
However, prevailing analysis and design procedures for massive MIMO are usually based on scalar-quantity, white-noised, far-field~\cite{liu2018novel}, discretized~\cite{goldsmith2003capacity}, monochromatic, and other non-physically consistent assumptions. 
\red{To be specific, most of the existing information-theoretic models may over-estimate the MIMO channel capacity because they usually assume that 1) noises at different receiver antennas are independent, and 2) the power of noise is independent to the number of antennas. } 
which will lead to mismatches between the system design and the actual propagation environment in the novel architectures beyond massive MIMO. 
Therefore, a theory that can model and analyze the real-world EM wireless information system with physically interpretable and mathematically reasonable assumptions is of interest, which motivates the research of EM information theory (EIT). 
...
\begin{figure}
	...
	\caption{In analogy with linear time-invariant systems described by a time-domain impulse response $h(t)$ in classical information theory, the EIT model is based on linear space-invariant systems described by the Green's function ${\bm G}({\bm r}, {\bm s})$.} 
	\label{fig:LTI_LSI}
\end{figure}
...
\red{Gaussian random processes, usually indexed by the continuous time $t$, are widely applied to model communication signals, from which entropy rates and mutual information formulas can be derived.} Gaussian random fields (GRFs) are generalized random processes that allow multiple index variables. 
...
Similarly, in order to capture the randomness of the transmitted and received signals, the current distributions at the transmitter and the electric fields at the receiver can also be modeled by GRFs~\cite{wan2022mutual}, \red{from which the information-theoretic mutual information can be defined to characterize the information transmission capability of such EM channels.} The GRF-based information-theoretic analysis will be explained in detain in Section.~\ref{sec_3_subsec_3}. 
...
In this part, we will discuss the channel models in EIT. The basic idea is to describe the EM channel by a \red{bounded linear operator $T$ that maps the source current distribution ${\bm J}({\bm s})\in\mathcal{L}^2(V_{\rm T})$ to the noiseless received field ${\bm E}({\bm r})\in\mathcal{L}^2(V_{\rm R})$.   }
...
The EIT channel can either be deterministically modeled or stochastically modeled. \red{Deterministic channel models have simpler mathematical expressions, but stochastic models usually better capture the small-scale fast fading caused by multipath effects and user mobility. }
...
Similarly, in EIT, the noise is usually modeled as a spatial AWGN with flat wavenumber PSD. This spatial AWGN implies that the additive noises at any disjoint small spatial regions $V_1, V_2$ are independent and identically distributed (i.i.d.) complex Gaussian random variables, \red{whose variances are proportional to the volumes $\mu(V_1), \mu(V_2)$ of the small regions.} Although the AWGN model facilitates theoretical analysis, the white spectral assumption is problematic, since it causes an unbounded noise power. 
...
Similar to the SVD decomposition of matrices, the DoF of a LoS EM channel can be solved by \red{expanding the channel operator $T$ onto a series of orthogonal sub-channels: $T=\sum_n \sigma_n |\varphi_n\rangle\langle\phi_n|$,} and counting the number of channel gains $\sigma_n$ that exceed a certain threshold $\epsilon$. 
...
Besides the DoF which reveals the number of available sub-channels, the channel capacity $I({\bm J}; {\bm E})$ is another important performance indicator showing the maximum information transmission rate of a wireless system. 
...
Different from the matrix determinant form of MIMO mutual information, the EIT information takes a Fredholm determinant form \red{$\log\det({\bf I}+T_{\bm E}T_{\bm N}^{-1})$~\cite{wan2022mutual}, }where $T_{\bm E}$ and $T_{\bm N}$ are self-adjoint autocorrelation operators of the random fields ${\bm E}({\bm r})$, and ${\bm N}({\bm r})$, respectively.  
...
In classical information theory, the ``capacity'' is defined as the supremum of all the operationally achievable transmission rates. \red{It is favorable that in discrete memoryless channels, the capacity equals the maximum mutual information. However, this conclusion has not been proved for continuous waveform channels.} In the existing works on EIT~\cite{wan2022mutual,zhang2022pdma,marzetta2022fourier}, maximum EIT mutual information values are calculated under the assumptions of linear deterministic EM channels and continuous transceivers, and the EIT mutual information is further compared to the discrete MIMO mutual information. Unfortunately, these EIT mutual information values only serve as an upper bound to the EM operational capacity. 
